...
class ReConnect:
    ...
    # 模拟登录
    def login(self):
        print self.getCurrentTime(), u"正在尝试认证无线网络"
        ip = self.getIP()
        data = {
    ...
    def main(self):
        ...
        while True:
            nowIP = self.getIP()
            if not nowIP:
                print self.getCurrentTime(), u"请检查是否正常连接无线网络"
            else:
                print self.getCurrentTime(), u"成功连接了网络,本机IP为", nowIP
                self.login()
                while True:
                    ...
                    if not can_connect:
                        nowIP = self.getIP()
                        if not nowIP:
                            print self.getCurrentTime(), u"当前已经掉线，请确保连接上了网络"
                        else:
